...
<?php
    ...
    if(!empty($dados['valRes'])){
        ...
        $result_val_resposta = $conexao->prepare($query_val_resposta);
        ...
        $result_pergunta = $conexao->prepare($query_pergunta);
...
<form method="POST" action="javascript.php">
    <?php
        if(($result_pergunta) AND $result_pergunta->rowCount() != 0){
            ...
            $result_resposta = $conexao->prepare($query_resposta);
    ...
    ?>
    <br>
    <input type="submit" name="valRes" value="Enviar">
</form>

<hr>
<a href="javascript.php">Próxima</a>
